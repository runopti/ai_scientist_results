...
class NeuralNetwork:
    ...
    def calc_minibatch(self, indata_input, indata_tch):
        self.output_calc(indata_input)
        self.teach_calc(indata_tch)
        # self.weight_calc()
        self.weight_calc_minibatch()
    ...
    def weight_calc_minibatch(self):
        for n in range(self.out_nodes):
            for k in range(self.in_nodes + 2, self.all_nodes + 2):
                for m in range(self.all_nodes + 2):
                    if self.w_ot_ot[n][k][m] != 0:
    ...
    def train_minibatch(self, input_data, target_data, epochs):
        train_acc = []
        num_total = len(input_data)
        for epoch in range(epochs):
            self.err = 0
...
def plot_2d_decision_boundary(
    ...
    plt.title(
        f"{dataset_name.capitalize()}, Act: {nn.act_func}, Test Acc: {test_acc:.2f}"
...
if __name__ == "__main__":
    ...
    for dataset_name in ["gaussian", "xor", "circle", "spiral"]:
        for act_func in ["sigmoid", "tanh", "relu"]:
            ...
            eval_target_data = eval_data[:, 2].reshape(-1, 1).tolist()
            eval_input_data_orig = eval_input_data
            if config.binary_repr:
            ...
            plot_info = plot_2d_decision_boundary(
                nn,
                np.array(eval_input_data_orig),
                np.array(eval_target_data),
                config.show_plot,
                "test.png",
                acc,
                binary_repr=config.binary_repr,
